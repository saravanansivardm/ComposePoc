...
import com.example.pocofdigivalapp.utils.getAllTabs
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext

class CustomProgressIndicator : ComponentActivity() {
        ...
        setContent {
            Column(modifier = Modifier) {
                ScrollableTabRow(
                    ...
                    for (category in getAllTabs()) {
//                        Log.e("category_log", category.toString())
                        Column(
    ...
    fun startThreadGradient() {
        viewModelScope.launch {
            withContext(Dispatchers.Default) {
                ...
                val totalDownloadSize = 89f
                var downloadedSize = 0f

                while (true) {

                    downloadedSize += ((1..50).random().toFloat())

                    if (downloadedSize < totalDownloadSize) {
                        withContext(Dispatchers.Main) {
                            downloadedPercentage.value =
                                ((downloadedSize / totalDownloadSize) * 100)
//                            Log.e("downloadedPercentage_log", downloadedPercentage.value.toString())
                        }
                    } else {
                        withContext(Dispatchers.Main) {
